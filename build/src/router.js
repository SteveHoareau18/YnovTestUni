...
const DatabaseService_1 = require("./service/DatabaseService");
class Router {
    constructor(app) {
        const db = new DatabaseService_1.default();
        this.booksRepository = new BooksRepository_1.default(db);
        this.app = app;
    }
    init() {
    ...
    scoreTokenizedMatch(text, tokenizedSearchText) {
    ...
    setupRoutes() {
        this.app.get("/books/search", (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const rawBooks = yield this.booksRepository.searchBooks();
                ...
                const scoredBooks = rawBooks.map((book) => {
                    ...
                    let score = this.scoreTokenizedMatch(book.title, tokenizedSearchText) * 4 +
                    ...
                    return Object.assign(Object.assign({}, book), { relevanceScore: score });
                });
                // 3. Filter and sort
        ...
        this.app.post("/book/:bookId/chapter", (req, res) => __awaiter(this, void 0, void 0, function* () {
            const bookId = Number.parseInt(req.params.bookId);
            let { title, content, chapterNumber } = req.body;
            try {
                yield this.booksRepository.createChapter(bookId, title, content, chapterNumber);
        ...
        this.app.get("books/:bookId/last-read-chapter", (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const [readingProgressRows] = yield connection.query(`
        ...
        `, [bookId, userId]);
                if (readingProgressRows.length === 0) {
                    ...
                    const [firstChapterRows] = yield connection.query(`
              SELECT chapters.*
              FROM chapters
              WHERE book_id = ?
          ...
          `, [bookId]);
                ...
                const [lastChapterReadRow] = yield connection.query(`
            SELECT chapters.*
            FROM chapters
            WHERE id = ?
        ...
        this.app.get("users/:userId/reading-list", (req, res) => __awaiter(this, void 0, void 0, function* () {
            const userId = req.params.userId;
            try {
                const readingList = yield this.booksRepository.fetchReadingList(userId);
        ...
        this.app.get("/books/:bookId/chapter/:chapterId", (req, res) => __awaiter(this, void 0, void 0, function* () {
            const bookId = req.params.bookId;
            ...
            try {
                const db = new DatabaseService_1.default();
                const connection = db.getConnection();
                const [chapterRows] = yield connection.query(`
            SELECT *
            FROM chapters
            WHERE book_id = ?
              AND id = ?
